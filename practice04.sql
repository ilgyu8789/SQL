...
WHERE department_id = (SELECT department_id -- 쿼리1을 넣음
                        FROM employees
...
WHERE location_id = (SELECT location_id 
                        FROM departments
                        WHERE department_id = 
                            (SELECT department_id -- 쿼리1을 넣음
                            FROM employees
                            WHERE first_name='Steven'   -- 쿼리2를 넣음
                                AND last_name='King')
                                );
                                
-- Q4.
-- 쿼리1.
SELECT salary FROM employees WHERE job_id='ST_MAN'; --ST MAN의 급여 정보
-- 최종쿼리
